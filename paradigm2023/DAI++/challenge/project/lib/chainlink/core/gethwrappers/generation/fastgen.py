...
for line in open(gogenpath):
    if abigenpath in line:
        ...
        srcpath = (
            os.path.abspath(os.path.join(godir, abipath))
            .replace("/abi/", "/src/")
            .replace(".json", ".sol")
        )
        if not os.path.exists(srcpath):
            srcpath = os.path.join(
                os.path.dirname(srcpath), "dev", os.path.basename(srcpath)
            )
        if not os.path.exists(srcpath):
            srcpath = srcpath.replace("/dev/", "/tests/")
        if os.path.basename(srcpath) != "OffchainAggregator.sol":
            assert os.path.exists(srcpath), "could not find " + os.path.basename(
                srcpath
            )
...
for pkgname in args:
    ...
    outpath = os.path.abspath(
        os.path.join(godir, "generated", pkgname, pkgname + ".go")
    )
